# ANOVA1f_indep_Welch_sodio_noutf.R
...
res <- jmv::anovaOneW(formula = Sodium~Instructor, data=TH,
                      desc=TRUE, descPlot = FALSE, phMethod ='gamesHowell',
                      phMeanDif = TRUE, phTest=TRUE, phFlag=TRUE)
cat("\n\t One-Way ANOVA (Welch's)\n")
print (res$anova$asDF)
cat("\n\tGroup Descriptives\n")
print (res$desc$asDF)
# post hoc
cat("\n\tGames-Howell Post-Hoc Test – Sodium\n", sep="")
posthoc <- res$postHoc[[1]]$asDF
rows <- nrow(posthoc)
table <- matrix('', nrow=2 + 5*rows, ncol=2 + rows)
posthoc <- res$postHoc[[1]]$asDF
for (r in 1:rows)
{
  table[1,r+2] <- as.character(posthoc[r,1])
}
for (r in 1:rows)
{
  table[3+5*(r-1), 1] <- as.character(posthoc[r,1])
  for (n in seq(2,8,2))
  {
    table[3+5*(r-1)+(n/2)-1, 2] <- as.character(posthoc[r,n])
    c2 <- 3
    for (c in seq(8+n/2,ncol(posthoc),4))
    {
      v <- as.numeric(as.character(posthoc[r,c]))
      if (!is.na(v)) {v <- round(v,4)} else {v <- "-"}
      table[3+5*(r-1)+(n/2)-1, c2] <- v
      c2 <- c2+1
    }
  }
}
prmatrix(table,rowlab=rep("",nrow(table)),collab=rep("",ncol(table)),quote=FALSE)
