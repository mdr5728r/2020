# -*- coding: utf-8 -*-
# Teste t relacionado unilateral a esquerda
# Referencial teorico da distribuicao t central e nao-central

n <- 16
mediaA <- 5.9; mediaB <- 4.8
dpA <- 1.2; dpB <- 1.8
r <- 0.3
alfa <- 0.05
dif <- mediaB - mediaA
...
tH1 <- rt(1e6, df, ncp)
dtH1 <- density(tH1)
# quantil critico (1-alfa)% sob H0 e poder 
q <- qt(alfa, df) 
poder <- pt(q,df,ncp)
...
# formato de p e poder
p_txt <- paste("p = ",round(p,4),sep="")
if(p<1e-4){p_txt <- paste("p = ",format(p, format = "e", digits = 2),sep="")}
if(p>0.9999){p_txt <- "p > 0.9999"}
plot(dtH0,type="l",
     main=paste("teste t relacionado unilateral a esquerda\n",
...
abline(v=q, lwd=1, lty = 3, col="#000000")
abline(v=t, lwd=2, lty = 2, col="#000000")
# area de alfa
# esquerda
alfa_x <- c(dtH0$x[dtH0$x<=q]) 
alfa_y <- c(dtH0$y[dtH0$x<=q])
...
# area beta (1-poder)
beta_x <- c(dtH1$x[dtH1$x>=q]) 
beta_y <- c(dtH1$y[dtH1$x>=q])
...
# esquerda
p_x <- c(dtH0$x[dtH0$x<=t]) 
p_y <- c(dtH0$y[dtH0$x<=t])
